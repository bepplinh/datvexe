...
export const SearchTripProvider = ({ children }) => {
    ...
    const [departDate, setDepartDate] = useState(
        savedState?.departDate || null
    );
    const [returnDate, setReturnDate] = useState(
        savedState?.returnDate || null
    );
    ...
    const handleSearchTrip = async () => {
        // --- Validation ---
        if (!from) return { success: false, message: "Vui lòng chọn điểm đi" };
        if (!to) return { success: false, message: "Vui lòng chọn điểm đến" };
        ...
        if (tripType === "roundtrip" && !finalReturnDate) {
            finalReturnDate = dayjs(finalDepartDate)
                .add(1, "day")
                .startOf("day");
